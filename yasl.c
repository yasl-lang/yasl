...
int YASL_setglobal(struct YASL_State *S, const char *name) {
	...
	struct YASL_String *string = YASL_String_new_copy(strlen(name), name);
...
int YASL_setmt(struct YASL_State *S) {
	...
	ud_setmt(&S->vm, vm_peek(&S->vm).value.uval, mt);
...
void YASL_pushlit(struct YASL_State *S, const char *value) {
	vm_pushstr((struct VM *) S, YASL_String_new_copy(strlen(value), value));
}
...
void YASL_pushlstr(struct YASL_State *S, const char *value, size_t len) {
	vm_pushstr((struct VM *)S, YASL_String_new_copy(len, value));
}

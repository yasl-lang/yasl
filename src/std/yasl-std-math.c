...
static yasl_float YASLX_checknnum(struct YASL_State *S, const char *name, unsigned n) {
	if (!YASL_isnnum(S, n)) {
		YASLX_print_and_throw_err_bad_arg_type_n(S, name, n, "float or int");
	}
...
static int YASL_math_abs(struct YASL_State *S) {
	...
	YASLX_print_and_throw_err_bad_arg_type_n(S,"math.abs", 0, YASL_NUM_NAME);
}
...
static int YASL_math_max(struct YASL_State *S) {
	...
	for (yasl_int i = 0; i < num_va_args; i++) {
		if (vm_isint((struct VM *)S)) {
			...
			YASLX_print_err_bad_arg_type(S, "math.max", (int)(num_va_args - i - 1), YASL_NUM_NAME, YASL_peektypename(S));
...
static int YASL_math_min(struct YASL_State *S) {
	...
	for (yasl_int i = 0; i < num_va_args; i++) {
		if (vm_isint((struct VM *)S)) {
			...
			YASLX_print_err_bad_arg_type(S,"math.min", (int)(num_va_args - i - 1), YASL_NUM_NAME, YASL_peektypename(S));
			YASLX_throw_err_type(S);
		}
	}
	vm_push((struct VM *)S, max);
...
static int YASL_math_isprime(struct YASL_State *S) {
	if (!YASL_isnnum(S, 0)) {
		YASLX_print_and_throw_err_bad_arg_type_n(S,"math.isprime", 0, YASL_NUM_NAME);
	}
...
static int YASL_math_gcd(struct YASL_State *S) {
	if (!YASL_isnnum(S, 1)) {
		YASLX_print_and_throw_err_bad_arg_type_n(S,"math.gcd", 1, YASL_NUM_NAME);
	}
	...
	if (!YASL_isnnum(S, 0)) {
		YASLX_print_and_throw_err_bad_arg_type_n(S,"math.gcd", 0, YASL_NUM_NAME);
	}
...
static int YASL_math_lcm(struct YASL_State *S) {
	if (!YASL_isnnum(S, 1)) {
		YASLX_print_and_throw_err_bad_arg_type_n(S,"math.lcm", 1, YASL_NUM_NAME);
	}
	...
	if (!YASL_isnnum(S, 0)) {
		YASLX_print_and_throw_err_bad_arg_type_n(S,"math.lcm", 0, YASL_NUM_NAME);
	}
...
static int YASL_math_clamp(struct YASL_State *S) {
	if (!YASL_isnnum(S, 2)) {
		YASLX_print_and_throw_err_bad_arg_type_n(S,"math.clamp", 2, YASL_NUM_NAME);
	}
	...
	if (!YASL_isnnum(S, 1)) {
		YASLX_print_and_throw_err_bad_arg_type_n(S,"math.clamp", 1, YASL_NUM_NAME);
	}
	...
	if (!YASL_isnnum(S, 0)) {
		YASLX_print_and_throw_err_bad_arg_type_n(S,"math.clamp", 0, YASL_NUM_NAME);
	}

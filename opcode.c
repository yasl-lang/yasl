...
MLC       = 0x50, // allocate memory (takes next 1 byte as type, next 8 bytes as length.
MCP_8     = 0x52, // copy from memory (takes next byte as offset, next 8 bytes as length, next <length> bytes as data.
SCP_8     = 0x53, // copy from top of stack to next on stack (takes next byte as offset, next 8 bytes as length.)

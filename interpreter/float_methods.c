...
int float_toint(struct YASL_State *S) {
	if (!YASL_isfloat(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "float.toint", 0, Y_FLOAT, YASL_peektype(S));
...
int float_tobool(struct YASL_State *S) {
	if (!YASL_isfloat(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "float.tobool", 0, Y_FLOAT, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
...
int float_tofloat(struct YASL_State *S) {
	if (!YASL_isfloat(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "float.tofloat", 0, Y_FLOAT, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
	return YASL_SUCCESS;
}
...
int float_tostr(struct YASL_State *S) {
	if (!YASL_isfloat(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "float.tostr", 0, Y_FLOAT, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
	yasl_float val = YASL_popfloat(S);

...
int str_repeat(struct YASL_State *S) {
	ASSERT_TYPE(S->vm, Y_INT, "str.repeat");
	yasl_int num = YASL_GETINT(vm_pop(S->vm));
	ASSERT_TYPE(S->vm, Y_STR, "str.repeat");
	String_t *string = YASL_GETSTR(vm_pop(S->vm));
	...
	vm_push(S->vm, YASL_STR(str_new_sized_heap(0, size, str)));
	return 0;
}

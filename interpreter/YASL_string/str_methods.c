...
int str___get(struct YASL_State *S) {
    ...
    if (index.type != Y_INT64) {
        ...
        if (YASL_GETINT(index) >= 0)
            vm_push(S->vm, YASL_STR(str_new_sized_from_mem(str->start + YASL_GETINT(index), str->start + YASL_GETINT(index) + 1, str->str)));
        else vm_push(S->vm, YASL_STR(str_new_sized_from_mem(str->start + YASL_GETINT(index) + yasl_string_len(str), str->start + YASL_GETINT(index) + yasl_string_len(str) + 1, str->str)));
    }
    return 0;
}
...
int str_slice(struct YASL_State *S) {
    ...
    if (!YASL_ISINT(start_index) || !YASL_ISINT(end_index)) {
        return -1;
    } else if (YASL_GETINT(start_index) < -yasl_string_len(str) || YASL_GETINT(start_index) > yasl_string_len(str)) {
        return -1;
    } else if (YASL_GETINT(end_index) < -yasl_string_len(str) || YASL_GETINT(end_index) > yasl_string_len(str)) {
        return -1;
    }

    int64_t start = YASL_GETINT(start_index) < 0 ? YASL_GETINT(start_index) + yasl_string_len(str) : YASL_GETINT(start_index);
    int64_t end = YASL_GETINT(end_index) < 0 ? YASL_GETINT(end_index) + yasl_string_len(str) : YASL_GETINT(end_index);
...
int str_replace(struct YASL_State *S) {
    ...
    String_t *replace_str = YASL_GETSTR(vm_pop(S->vm));
    ASSERT_TYPE(S->vm, Y_STR, "str.replace");
    String_t *search_str = YASL_GETSTR(vm_pop(S->vm));
    ASSERT_TYPE(S->vm, Y_STR, "str.replace");
    String_t *str = YASL_GETSTR(vm_pop(S->vm));
...
int str_ltrim(struct YASL_State *S) {
    ...
    vm_push(S->vm, YASL_STR(str_new_sized_from_mem(haystack->start + start, haystack->start + yasl_string_len(haystack),
                                                                   haystack->str)));
...
int str_rtrim(struct YASL_State *S) {
    ...
    vm_push(S->vm, YASL_STR(str_new_sized_from_mem(haystack->start, haystack->start + end, haystack->str)));
...
int str_trim(struct YASL_State *S) {
    ...
    vm_push(S->vm, YASL_STR(str_new_sized_from_mem(haystack->start + start, haystack->start + end, haystack->str)));

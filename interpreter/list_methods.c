...
int list___get(struct YASL_State *S) {
	...
	if (!YASL_top_islist(S)) {
		YASL_PRINT_ERROR_BAD_ARG_TYPE("list.__get", 0, Y_LIST, YASL_top_peektype(S));
...
int list___set(struct YASL_State *S) {
	...
	struct YASL_Object index = vm_pop((struct VM *) S);
	if (!YASL_top_islist(S)) {
		YASL_PRINT_ERROR_BAD_ARG_TYPE("list.__set", 0, Y_LIST, YASL_top_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_List *ls = YASL_GETLIST(vm_pop((struct VM *) S));
	if (!YASL_ISINT(index)) {
		YASL_PRINT_ERROR_BAD_ARG_TYPE("list.__set", 1, Y_INT, index.type);
		return YASL_TYPE_ERROR;
	} else if (YASL_GETINT(index) < -(int64_t) ls->count || YASL_GETINT(index) >= (int64_t) ls->count) {
		// TODO: fix this
...
int list_tostr(struct YASL_State *S) {
	if (!YASL_top_islist(S)) {
		YASL_PRINT_ERROR_BAD_ARG_TYPE("list.tostr", 0, Y_LIST, YASL_top_peektype(S));
		return YASL_TYPE_ERROR;
	}
...
int list_push(struct YASL_State *S) {
	...
	if (!YASL_top_islist(S)) {
		YASL_PRINT_ERROR_BAD_ARG_TYPE("list.push", 0, Y_LIST, YASL_top_peektype(S));
		return YASL_TYPE_ERROR;
	}
...
int list_copy(struct YASL_State *S) {
	if (!YASL_top_islist(S)) {
		YASL_PRINT_ERROR_BAD_ARG_TYPE("list.copy", 0, Y_LIST, YASL_top_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_List *ls = YASL_GETLIST(vm_pop((struct VM *) S));
...
int list___add(struct YASL_State *S) {
	if (!YASL_top_islist(S)) {
		YASL_PRINT_ERROR_BAD_ARG_TYPE("list.__add", 1, Y_LIST, YASL_top_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_List *b = YASL_GETLIST(vm_pop((struct VM *) S));
	if (!YASL_top_islist(S)) {
		YASL_PRINT_ERROR_BAD_ARG_TYPE("list.__add", 0, Y_LIST, YASL_top_peektype(S));
		return YASL_TYPE_ERROR;
	}
...
int list_extend(struct YASL_State *S) {
	if (!YASL_top_islist(S)) {
		YASL_PRINT_ERROR_BAD_ARG_TYPE("list.extend", 1, Y_LIST, YASL_top_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_List *extend_ls = YASL_GETLIST(vm_pop((struct VM *) S));
	if (!YASL_top_islist(S)) {
		YASL_PRINT_ERROR_BAD_ARG_TYPE("list.extend", 0, Y_LIST, YASL_top_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_List *ls = YASL_GETLIST(vm_pop((struct VM *) S));
...
int list_pop(struct YASL_State *S) {
	if (!YASL_top_islist(S)) {
		YASL_PRINT_ERROR_BAD_ARG_TYPE("list.pop", 0, Y_LIST, YASL_top_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_List *ls = YASL_GETLIST(vm_pop((struct VM *) S));
	if (ls->count == 0) {
		YASL_PRINT_ERROR("ValueError: %s expected nonempty list as arg 0.\n", "list.pop");
...
int list_search(struct YASL_State *S) {
	...
	if (!YASL_top_islist(S)) {
		YASL_PRINT_ERROR_BAD_ARG_TYPE("list.search", 0, Y_LIST, YASL_top_peektype(S));
		return YASL_TYPE_ERROR;
	}
...
int list_reverse(struct YASL_State *S) {
	if (!YASL_top_islist(S)) {
		YASL_PRINT_ERROR_BAD_ARG_TYPE("list.reverse", 0, Y_LIST, YASL_top_peektype(S));
		return YASL_TYPE_ERROR;
	}
...
int list_slice(struct YASL_State *S) {
	...
	if (!YASL_top_islist(S)) {
		YASL_PRINT_ERROR_BAD_ARG_TYPE("list.slice", 0, Y_LIST, YASL_top_peektype(S));
		return YASL_TYPE_ERROR;
	}
...
int list_clear(struct YASL_State *S) {
	if (!YASL_top_islist(S)) {
		YASL_PRINT_ERROR_BAD_ARG_TYPE("list.clear", 0, Y_LIST, YASL_top_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_List *list = vm_poplist((struct VM *) S);
...
int list_join(struct YASL_State *S) {
	if (!YASL_ISSTR(vm_peek((struct VM *) S))) {
		YASL_PRINT_ERROR_BAD_ARG_TYPE("list.join", 1, Y_STR, YASL_top_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_String *string = vm_peekstr((struct VM *) S, S->vm.sp);
	S->vm.sp--;
	if (!YASL_top_islist(S)) {
		YASL_PRINT_ERROR_BAD_ARG_TYPE("list.join", 0, Y_LIST, YASL_top_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_List *list = vm_peeklist((struct VM *) S, S->vm.sp);
	S->vm.sp++;
...
int list_sort(struct YASL_State *S) {
	if (!YASL_top_islist(S)) {
		YASL_PRINT_ERROR_BAD_ARG_TYPE("list.sort", 0, Y_LIST, YASL_top_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_List *list = vm_poplist((struct VM *) S);

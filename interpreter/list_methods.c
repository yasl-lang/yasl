...
int list___get(struct YASL_State *S) {
	...
	if (!YASL_islist(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "list.__get", 0, Y_LIST, YASL_peektype(S));
...
int list___set(struct YASL_State *S) {
	...
	if (!YASL_isint(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "list.__set", 1, Y_INT, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}

	yasl_int index = YASL_popint(S);
	if (!YASL_islist(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "list.__set", 0, Y_LIST, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
...
int list_tostr(struct YASL_State *S) {
	if (!YASL_islist(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "list.tostr", 0, Y_LIST, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
...
int list_push(struct YASL_State *S) {
	...
	if (!YASL_islist(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "list.push", 0, Y_LIST, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
...
int list_copy(struct YASL_State *S) {
	if (!YASL_islist(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "list.copy", 0, Y_LIST, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
...
int list___add(struct YASL_State *S) {
	if (!YASL_islist(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "list.__add", 1, Y_LIST, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_List *b = YASL_GETLIST(vm_pop((struct VM *) S));
	if (!YASL_islist(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "list.__add", 0, Y_LIST, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
...
int list_extend(struct YASL_State *S) {
	if (!YASL_islist(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "list.extend", 1, Y_LIST, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_List *extend_ls = YASL_GETLIST(vm_pop((struct VM *) S));
	if (!YASL_islist(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "list.extend", 0, Y_LIST, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_List *ls = YASL_GETLIST(vm_pop((struct VM *) S));
...
int list_pop(struct YASL_State *S) {
	if (!YASL_islist(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "list.pop", 0, Y_LIST, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_List *ls = YASL_GETLIST(vm_pop((struct VM *) S));
	if (ls->count == 0) {
...
int list_search(struct YASL_State *S) {
	...
	if (!YASL_islist(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "list.search", 0, Y_LIST, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
...
int list_reverse(struct YASL_State *S) {
	if (!YASL_islist(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "list.reverse", 0, Y_LIST, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_List *ls = vm_poplist((struct VM *) S);
	YASL_reverse(ls);
...
int list_clear(struct YASL_State *S) {
	if (!YASL_islist(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "list.clear", 0, Y_LIST, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
...
int list_join(struct YASL_State *S) {
	if (!YASL_ISSTR(vm_peek((struct VM *) S))) {
		vm_print_err_bad_arg_type((struct VM *)S, "list.join", 1, Y_STR, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_String *string = vm_peekstr((struct VM *) S, S->vm.sp);
	S->vm.sp--;
	if (!YASL_islist(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "list.join", 0, Y_LIST, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_List *list = vm_peeklist((struct VM *) S, S->vm.sp);
	S->vm.sp++;
...
int list_sort(struct YASL_State *S) {
	if (!YASL_islist(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "list.sort", 0, Y_LIST, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_List *list = vm_poplist((struct VM *) S);

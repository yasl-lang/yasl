...
int int_toint(struct YASL_State *S) {
	if (!YASL_isint(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "int.toint", 0, Y_INT, YASL_peektype(S));
...
int int_tobool(struct YASL_State *S) {
	if (!YASL_isint(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "int.tobool", 0, Y_INT, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
...
int int_tofloat(struct YASL_State *S) {
	if (!YASL_isint(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "int.tofloat", 0, Y_INT, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
...
int int_tostr(struct YASL_State *S) {
	if (!YASL_isint(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "int.tostr", 0, Y_INT, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}

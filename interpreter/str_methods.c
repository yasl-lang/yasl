...
int str___get(struct YASL_State *S) {
	...
	if (!YASL_isstr(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "str.__get", 0, Y_STR, YASL_peektype(S));
...
int str_tofloat(struct YASL_State *S) {
	if (!YASL_isstr(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "str.tofloat", 0, Y_STR, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_String *str = YASL_GETSTR(vm_pop((struct VM *) S));
...
int str_toint(struct YASL_State *S) {
	if (!YASL_isstr(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "str.toint", 0, Y_STR, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
...
int str_tobool(struct YASL_State* S) {
	if (!YASL_isstr(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "str.tobool", 0, Y_STR, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
...
int str_tostr(struct YASL_State *S) {
	if (!YASL_isstr(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "str.tostr", 0, Y_STR, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
	return YASL_SUCCESS;
}
...
int str_toupper(struct YASL_State *S) {
	if (!YASL_isstr(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "str.toupper", 0, Y_STR, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_String *a = YASL_GETSTR(vm_pop((struct VM *) S));
...
int str_tolower(struct YASL_State *S) {
	if (!YASL_isstr(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "str.tolower", 0, Y_STR, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_String *a = YASL_GETSTR(vm_pop((struct VM *) S));
...
int str_isalnum(struct YASL_State *S) {
	if (!YASL_isstr(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "str.isalnum", 0, Y_STR, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_String *a = YASL_GETSTR(vm_pop((struct VM *) S));
...
int str_isal(struct YASL_State *S) {
	if (!YASL_isstr(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "str.isal", 0, Y_STR, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_String *a = YASL_GETSTR(vm_pop((struct VM *) S));
...
int str_isnum(struct YASL_State *S) {
	if (!YASL_isstr(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "str.isnum", 0, Y_STR, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_String *a = YASL_GETSTR(vm_pop((struct VM *) S));
...
int str_isspace(struct YASL_State *S) {
	if (!YASL_isstr(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "str.isspace", 0, Y_STR, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_String *a = YASL_GETSTR(vm_pop((struct VM *) S));
...
int str_startswith(struct YASL_State *S) {
	if (!YASL_isstr(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "str.startswith", 1, Y_STR, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_String *needle = YASL_GETSTR(vm_pop((struct VM *) S));
	if (!YASL_isstr(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "str.startswith", 0, Y_STR, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
...
int str_endswith(struct YASL_State *S) {
	if (!YASL_isstr(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "str.endswith", 1, Y_STR, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_String *needle = YASL_GETSTR(vm_pop((struct VM *) S));
	if (!YASL_isstr(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "str.endswith", 0, Y_STR, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_String *haystack = YASL_GETSTR(vm_pop((struct VM *) S));
...
static int str_replace_default(struct YASL_State *S) {
	if (!YASL_isstr(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "str.replace", 2, Y_STR, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_String *replace_str = YASL_GETSTR(vm_pop((struct VM *) S));
	if (!YASL_isstr(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "str.replace", 1, Y_STR, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_String *search_str = YASL_GETSTR(vm_pop((struct VM *) S));
	if (!YASL_isstr(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "str.replace", 0, Y_STR, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_String *str = YASL_GETSTR(vm_pop((struct VM *) S));
...
int str_replace(struct YASL_State *S) {
	...
	if (!YASL_isint(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "str.replace", 3, Y_INT, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
...
int str_search(struct YASL_State *S) {
	if (!YASL_isstr(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "str.search", 1, Y_STR, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_String *needle = vm_popstr((struct VM *) S);
	if (!YASL_isstr(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "str.search", 0, Y_STR, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
...
int str_count(struct YASL_State *S) {
	if (!YASL_isstr(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "str.count", 1, Y_STR, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_String *needle = vm_popstr((struct VM *) S);
	if (!YASL_isstr(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "str.count", 0, Y_STR, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_String *haystack = vm_popstr((struct VM *) S);
...
static int str_split_default(struct YASL_State *S) {
	if (!YASL_isstr(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "str.split", 0, Y_STR, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_String *haystack = YASL_GETSTR(vm_pop((struct VM *) S));
...
int str_split(struct YASL_State *S) {
	...
	if (!YASL_isstr(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "str.split", 1, Y_STR, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_String *needle = YASL_GETSTR(vm_pop((struct VM *) S));
	if (!YASL_isstr(S)) {
...
static int str_ltrim_default(struct YASL_State *S) {
	if (!YASL_isstr(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "str.ltrim", 0, Y_STR, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
...
int str_ltrim(struct YASL_State *S) {
	...
	if (!YASL_isstr(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "str.ltrim", 1, Y_STR, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_String *needle = YASL_GETSTR(vm_pop((struct VM *) S));
	if (!YASL_isstr(S)) {
...
static int str_rtrim_default(struct YASL_State *S) {
	if (!YASL_isstr(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "str.rtrim", 0, Y_STR, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_Object top = vm_peek((struct VM *) S);
	inc_ref(&top);
...
int str_rtrim(struct YASL_State *S) {
	...
	if (!YASL_isstr(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "str.rtrim", 1, Y_STR, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_String *needle = YASL_GETSTR(vm_pop((struct VM *) S));
	if (!YASL_isstr(S)) {
...
static int str_trim_default(struct YASL_State *S) {
	if (!YASL_isstr(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "str.trim", 0, Y_STR, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_Object top = vm_peek((struct VM *) S);
	inc_ref(&top);
...
int str_trim(struct YASL_State *S) {
	...
	if (!YASL_isstr(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "str.trim", 1, Y_STR, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
	struct YASL_String *needle = YASL_GETSTR(vm_pop((struct VM *) S));
	if (!YASL_isstr(S)) {
...
int str_repeat(struct YASL_State *S) {
	if (!YASL_ISINT(vm_peek((struct VM *) S))) {
		vm_print_err_bad_arg_type((struct VM *)S, "str.rep", 1, Y_INT, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}
	yasl_int num = YASL_GETINT(vm_pop((struct VM *) S));
	if (!YASL_isstr(S)) {
		vm_print_err_bad_arg_type((struct VM *)S, "str.rep", 0, Y_STR, YASL_peektype(S));
		return YASL_TYPE_ERROR;
	}

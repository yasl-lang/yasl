...
static struct RC_UserData **builtins_htable_new(struct VM *const vm) {
	...
	ht[Y_UNDEF] = ud_new(undef_builtins(vm), "table", NULL, rcht_del_data);
	ht[Y_UNDEF]->rc->refs++;
	ht[Y_FLOAT] = ud_new(float_builtins(vm), "table", NULL, rcht_del_data);
	ht[Y_FLOAT]->rc->refs++;
	ht[Y_INT] = ud_new(int_builtins(vm), "table",NULL, rcht_del_data);
	ht[Y_INT]->rc->refs++;
	ht[Y_BOOL] = ud_new(bool_builtins(vm), "table", NULL, rcht_del_data);
	ht[Y_BOOL]->rc->refs++;
	ht[Y_STR] = ud_new(str_builtins(vm), "table", NULL, rcht_del_data);
	ht[Y_STR]->rc->refs++;
	ht[Y_LIST] = ud_new(list_builtins(vm), "table", NULL, rcht_del_data);
	ht[Y_LIST]->rc->refs++;
	ht[Y_TABLE] = ud_new(table_builtins(vm), "table", NULL, rcht_del_data);
	ht[Y_TABLE]->rc->refs++;
	return ht;
}

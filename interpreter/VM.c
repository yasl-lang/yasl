...
static struct RC_UserData **builtins_htable_new(struct VM *const vm) {
	...
	ht[Y_UNDEF] = ud_new(undef_builtins(vm), T_TABLE, "table", NULL, rcht_del_data);
	ht[Y_UNDEF]->rc->refs++;
	ht[Y_FLOAT] = ud_new(float_builtins(vm), T_TABLE, "table", NULL, rcht_del_data);
	ht[Y_FLOAT]->rc->refs++;
	ht[Y_INT] = ud_new(int_builtins(vm), T_TABLE, "table",NULL, rcht_del_data);
	ht[Y_INT]->rc->refs++;
	ht[Y_BOOL] = ud_new(bool_builtins(vm), T_TABLE, "table", NULL, rcht_del_data);
	ht[Y_BOOL]->rc->refs++;
	ht[Y_STR] = ud_new(str_builtins(vm), T_TABLE, "table", NULL, rcht_del_data);
	ht[Y_STR]->rc->refs++;
	ht[Y_LIST] = ud_new(list_builtins(vm), T_TABLE, "table", NULL, rcht_del_data);
	ht[Y_LIST]->rc->refs++;
	ht[Y_TABLE] = ud_new(table_builtins(vm), T_TABLE, "table", NULL, rcht_del_data);
	ht[Y_TABLE]->rc->refs++;
	return ht;
}
...
static void vm_fdiv(struct VM *const vm) {
	...
	if (obj_isnum(&left) && obj_isnum(&right)) {
		...
		vm_call_method_now_2(vm, left, right, overload_name, "/ not supported for operands of types %s and %s.",
					  obj_typename(&left),
					  obj_typename(&right));
	}
}
...
void vm_EQ(struct VM *const vm) {
	...
	if (obj_isuserdata(&a) && obj_isuserdata(&b) ||
		...
		vm_call_method_now_2(vm, a, b, "__eq", "== not supported for operands of types %s and %s.", obj_typename(&a), obj_typename(&b));
	} else {
